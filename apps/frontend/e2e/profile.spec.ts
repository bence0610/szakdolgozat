 ...
 *  6. Active ticket card contains match info, seat info, price, and QR code
...
test.describe('Profile Page – /profile', () => {
  ...
  test('should show match info and seat info inside the active ticket card', async ({ page }) => {
    await page.goto('/profile');
    const profile = new ProfilePage(page);
    ...
    // QR code
    await expect(card).toContainText(TICKET_ACTIVE.qrCode);
  });
